...
namespace K2LinCrypto
{
    public partial class Crypto : Form
    {
        ...
        public Crypto()
        {
            ...
            SelfSessionID.Text = Math.Abs(PubKeyXMLformat.GetHashCode()).ToString();
        }
        ...
        private void ReceiverPublic_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("发现错误了！" + error + " 没有识别到公钥! 您输入的是什么东西捏", "K2Lin Crypto (ﾟДﾟ*)ﾉ", MessageBoxButtons.OK);
            }
        }
        ...
        private void GenerateAES_KEY_Click(object sender, EventArgs e)
        {
            ...
            Clipboard.SetData(DataFormats.Text, (Object)Convert.ToBase64String(key));
            MessageBox.Show("已生成密钥并复制到剪贴板！", "K2Lin Crypto ε=( o｀ω′)ノ", MessageBoxButtons.OK);
        }
    }
}
